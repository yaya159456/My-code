'''
    ...
    利用TensorFlow自带的可视化工具TensorBoard进行可视化。这三种方法，在前面博客中都有过比较详细的介绍。
    但是，TensorFlow中最重要的可视化方法是通过tensorBoard、tf.summary和tf.summary.FileWriter这三个模块相互合作来完成的。
    tf.summary:https://www.2cto.com/kf/201805/746214.html
'''

import tensorflow as tf
from tensorflow.examples.tutorials.mnist import input_data

SUM_MARY_DIR = r"F:\Pycharm\venv\Scripts\logs"
MNIST_DIR = r'F:\Pycharm\yaya\YaYa\TensorFlow\MNIST_data'
BATCH_SIZE = 100
...
def main(_):
    ...
    # sess.run函数来明确调用这些函数。Tensorflow提供了tf.merge_all_summaries函数来整理所有的日志生成操作。
    # 在Tensorflow程序执行的过程中只需要运行这个操作就可以将代码中定义的所有日志生成操作执行一次，从而将所有日志写入文件
    merged = tf.summary.merge_all()

    with tf.Session() as sess:
        # 初始化写文件的writer，并将当前计算图写入日志
        summary_writer = tf.summary.FileWriter(SUM_MARY_DIR, sess.graph)
        tf.global_variables_initializer().run()

        for i in range(TRAINING_STEPS):
            xs, ys = mnist.train.next_batch(BATCH_SIZE)#next_batch 随机取xx个训练样本
